#!/usr/bin/env python3
"""
Quick Test V14 - Test Multiple V14 Models

Tests V14 detector + V14 OCR with different model sizes.
...
def test_model_combination(detector_model, ocr_model, sample_path, user_name, serial_key, signature, region="univ"):
    """Test a specific detector + OCR combination"""
    from marearts_anpr import ma_anpr_detector_v14, ma_anpr_ocr_v14
    from marearts_anpr import marearts_anpr_from_image_file
    
    print(f"\n{'='*70}")
    print(f"Testing: Detector={detector_model}, OCR={ocr_model}, Region={region}")
    print(f"{'='*70}")
    ...
    try:
        ...
        print(f"✅ OCR loaded ({ocr_time:.2f}s)")
...
def main():
    print("\n" + "="*70)
    print("MAREARTS ANPR - Quick Test V14 (Multiple Models)")
    print("="*70)
    print("\nTests V14 detector + V14 OCR with different model sizes")
